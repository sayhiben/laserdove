...
def test_wait_for_ready_polls_until_idle_and_updates_position():
    ...
    def build_snapshot(laser: RuidaLaser, status_bits: int, x_mm: float, y_mm: float, z_mm: float):
        return [
            ...
            (bytes([RuidaLaser.ACK]), (laser.host, laser.port)),
            (_make_swizzled_response(laser, RuidaLaser.MEM_CURRENT_Z, laser._encode_abscoord_mm(z_mm)), (laser.host, laser.port)),
        ]

    laser = RuidaLaser("127.0.0.1", socket_factory=lambda *_, **__: FakeSocket([]))
    busy_snapshot = build_snapshot(laser, RuidaLaser.STATUS_BIT_MOVING, 10.0, 20.0, 5.0)
    ready_snapshot = build_snapshot(laser, 0, 11.0, 21.0, 6.5)
